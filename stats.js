...
(function (root, factory) {
  ...
  function tick(stats, sessions, now = Date.now(), elapsedMs = 0, maxGapMs = 60000) {
    ...
    if (sessions.some((s) => (Array.isArray(s.agents) && s.agents.length) || s.mode === 'ralph')) {
      const d = dayOf(stats, key);
      if (d.agents == null) { d.agents = 0; d.ralphIters = 0; } // day written by an older build
      accrueAgents(stats, sessions, now, key, d);
    }
    if (elapsedMs <= 0 || elapsedMs > maxGapMs) return stats;
    const day = dayOf(stats, key);
