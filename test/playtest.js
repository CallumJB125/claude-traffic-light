...
(async () => {
  ...
  try {
    ...
    ok(Array.from($('cameos').querySelectorAll('.posebtn')).length >= 10, 'all 9 cameos + keep are offered (plus any photo faces)');
    ok(!!$('cameos').querySelector('.addface'), 'the Cameo row offers Add from photo');
    $('cameos').querySelector('.addface').click();
    ok(!$('face-modal').hidden && document.activeElement === $('face-choose'), 'Add from photo opens the face modal, focus on Choose file');
    ok($('face-replace').options.length === 9, 'the modal offers replacing each of the eight built-ins');
    // paste a drawn image: it loads into the crop, the preview wears it
    const art = document.createElement('canvas');
    art.width = 300; art.height = 200;
    const g2 = art.getContext('2d');
    g2.fillStyle = '#6fa8dc'; g2.fillRect(0, 0, 300, 200); g2.fillStyle = '#f2c9a0'; g2.beginPath(); g2.ellipse(150, 100, 50, 64, 0, 0, Math.PI * 2); g2.fill();
    const blob = await new Promise((res) => art.toBlob(res, 'image/png'));
    const dt = new DataTransfer();
    dt.items.add(new File([blob], 'image.png', { type: 'image/png' }));
    document.dispatchEvent(new ClipboardEvent('paste', { clipboardData: dt, bubbles: true, cancelable: true }));
    const wearing = () => document.querySelector('#face-preview svg').classList.contains('has-photo');
    for (let i = 0; i < 30 && !wearing(); i += 1) await sleep(100);
    ok(!$('face-frame').hidden && $('face-empty').hidden && !$('face-save').disabled, 'pasting an image loads it into the crop');
    const crop = $('face-crop').getBoundingClientRect();
    ok(Math.abs(crop.width - crop.height) < 1 && Math.abs(crop.height - $('face-img').getBoundingClientRect().height * 0.6) < 2, 'the crop starts square at 60% of the short side');
    ok(wearing(), 'the preview rig wears the draft face');
    ok($('face-name').value === '', 'a clipboard file name is not used as the face name');
    $('face-save').click();
    await sleep(50);
